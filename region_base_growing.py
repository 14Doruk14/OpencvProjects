...
class regionGrow():
    ...
    def readImage(self, img_path):
        ...
        if img is None:
            raise FileNotFoundError("Görüntü okunamadı. Dosya yolunu kontrol et.")
    ...
    def display(self):
        original = self.im.astype(np.uint8)
        segmented = self.SEGS

        combined = np.hstack((original, segmented))

        cv2.imshow("Original Image (Left) | Segmented Image (Right)", combined)
        cv2.waitKey(0)
...
exemple = regionGrow("tower image.jpeg", "20")
